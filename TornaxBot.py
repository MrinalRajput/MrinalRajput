...
async def create_db_pool():
    bot.pg_con = await asyncpg.create_pool(dsn="postgres://tvtwhdurxkloex:878e12f9f6c7ef192eaf2c11f69796a3a1e388d3da8de8338b72c873643db490@ec2-3-222-49-168.compute-1.amazonaws.com:5432/dckpp9l469cj78")
...
@bot.command()
async def unban(ctx, user: Optional[discord.User]=None):
    global embedContent
    try:
        if user is not None:
            if ctx.author.guild_permissions.ban_members:
                ...
                if member != ctx.author:
                    ...
                    if duration is not None and unit is not None:
                        if unit == "s" or "sec" in unit:
                            ...
                            dmAlert = f"You are Muted in {ctx.guild.name} Server by {ctx.author} for `{duration}` Seconds"if reason is None else f"You are Muted in {ctx.guild.name} Server by {ctx.author} for `{duration}`` Seconds\n\t With the Reason of {reason}"
                            ...
                            dmAlert = f"You are Muted in {ctx.guild.name} Server by {ctx.author} for `{duration}` Minutes"if reason is None else f"You are Muted in {ctx.guild.name} Server by {ctx.author} for `{duration}`` Minutes\n\t With the Reason of {reason}"
                            ...
                            dmAlert = f"You are Muted in {ctx.guild.name} Server by {ctx.author} for `{duration}` Hours"if reason is None else f"You are Muted in {ctx.guild.name} Server by {ctx.author} for `{duration}`` Hours\n\t With the Reason of {reason}"
                        if not member.guild_permissions.administrator:
                            ...
                            for channel in ctx.guild.channels:
                                await channel.set_permissions(mutedRole, speak=False, send_messages=False, add_reactions=False)
                            await member.send(embed=discord.Embed(description=dmAlert, color=embedTheme))
                        ...
                        if not member.guild_permissions.administrator:
                            ...
                            for channel in ctx.guild.channels:
                                await channel.set_permissions(mutedRole, speak=False, send_messages=False, add_reactions=False)
                            await member.send(embed=discord.Embed(description=f"You are Muted in {ctx.guild.name} Server by {ctx.author}"if reason is None else f"You are Muted in {ctx.guild.name} Server by {ctx.author}\n\t With the Reason of {reason}",color=embedTheme))
...
async def warn(ctx, member: Optional[discord.Member]=None, *, reason=None):
    if member is not None:
        ...
        await member.send(embed= discord.Embed(description=f"You are Warned in {ctx.guild.name} Server by {ctx.author}" if reason is None else f"You are Warned in {ctx.guild.name} Server by {ctx.author} \n\t Reason: {reason}",color=embedTheme))
        if reason is None:
    ...
    if ctx.author.guild_permissions.manage_messages:
        ...
        if word is not None:
            if word.lower() not in restricteds[ctx.guild.id]:
                ...
                await ctx.reply(embed=discord.Embed(description=f":exclamation: {word} is Already in Restricted Words!", color=embedTheme))
        else:
            await ctx.reply(f"Please Mention the Word You want to Ban/Restrict from the Server")
    else:
        await ctx.reply(embed=discord.Embed(description=f"You Must have `Manage Messages` Permissions to do that!", color=embedTheme))
...
@bot.command()
async def restricts(ctx):
    if ctx.guild:
        if ctx.author.guild_permissions.manage_messages:
            if ctx.guild.id not in restricteds:
                restricteds[ctx.guild.id] = []
            allwords = []
            for words in restricteds[ctx.guild.id]:
                allwords.append(f"`{words}`")
            if len(allwords) > 0:
                await ctx.send(embed=discord.Embed(title=f"Restricted Words in {ctx.guild} [{len(allwords)}]", description=", ".join(allwords), color=embedTheme))
            else:
                await ctx.reply(embed=discord.Embed(description=f":exclamation: The Server don't have Any Custom Banned/Restricted Word!", color=embedTheme))
        else:
            await ctx.reply(embed=discord.Embed(description=f"You Must have `Manage Messages` Permissions to do that!", color=embedTheme))

restrictshelp = f"restricts"
...
async def setnick(ctx, member: Optional[discord.Member]=None, *, newname):
    ...
    try:
        ...
        print(f"Exception - {e}")
        await ctx.reply(embed=discord.Embed(description="Failed Because That User Have Higher or Equal Role than my Role!", color=embedTheme))
        return
        ...
        if msg is not None:
            ...
            if ctx.guild.id in gActive:
                for gives in gActive[ctx.guild.id].keys():
                    if gActive[ctx.guild.id][gives]["message"] == msg:
                        thisgives = gives
    ...
    if role is not None:
        ...
        if role in member.roles:
            ...
            try:
                ...
                print(f"Exception - {e}")
                await ctx.reply(embed=discord.Embed(description="Failed to do that! The Role is Higher or Equal than My Role", color=embedTheme))
                return
...
async def choose(ctx, option1: Optional[str]=None, option2: Optional[str]=None):
    if option1 is None or option2 is None:
        ...
        choosed = random.choice(options)
        await ctx.reply(embed=discord.Embed(description=f"I Choose - {choosed}!", color=embedTheme))

choosehelp = f"choose <First Thing> <Second Thing>"
                ...
                try:
                    ...
                    if pokename.content.lower() not in toldpoke:
                        if pokename.content.lower() in pokelist:
                            ...
                            playersare.remove(turn.mention)
                            await ctx.send(f"Loser - {turn.mention}\nWinners - {', '.join(playersare)}")
                            pokegames[ctx.guild.id].remove(player1.id)
                            pokegames[ctx.guild.id].remove(player2.id)
                            if player3 is not None:
...
async def whois(ctx, member: Optional[discord.Member]=None):
    if ctx.guild:
        ...
        userEmbed.add_field(name=f"Joined {ctx.guild.name}", value= f"<t:{int(member.joined_at.timestamp())}:f>",inline=True)
        userEmbed.add_field(name=f"Joined Discord", value= f"<t:{int(member.created_at.timestamp())}:f>",inline=True)
...
async def serverinfo(ctx):
    if ctx.guild:
        ...
        serverEmbed.add_field(name="Created at", value=f"<t:{int(ctx.guild.created_at.timestamp())}:f>", inline=True)
...
async def info(ctx):
    ...
    infoembed.add_field(name="Prefix", value=DEFAULT_PREFIX, inline=True)
...
async def help(ctx, anycommand: Optional[str]=None):
    ...
    if anycommand is None:
        ...
        elif anycommand == "restricts": content=restrictshelp
