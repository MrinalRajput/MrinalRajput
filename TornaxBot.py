...
@bot.command()
async def football(ctx, player1: Optional[discord.Member]=None, player2: Optional[discord.Member]=None):
    global footballMatch ,footballCode
    if ctx.guild.id not in footballMatch:
        footballMatch[ctx.guild.id] = {}
    if ctx.guild.id not in footballCode:
        footballCode[ctx.guild.id] = {}
    ...
    if player1 is not None:
        if player2 is None:
            ...
            player1 = ctx.author
        if player2 != player1:
            if player1.id not in footballMatch[ctx.guild.id].keys() and player1.id not in footballMatch[ctx.guild.id].values():
                if player2.id not in footballMatch[ctx.guild.id].keys() and player2.id not in footballMatch[ctx.guild.id].values():
                    ...
                    footballMatch[ctx.guild.id][codeGenerator]["type"] = {}
                    await ctx.send(f"**🥅 Football ⚽**")
                    footballMatch[ctx.guild.id][codeGenerator]["type"][player1.id] = "attacker"
                    footballMatch[ctx.guild.id][codeGenerator]["type"][player2.id] = "goalkeeper"
                ...
                if player1 == ctx.author:
                    await ctx.send(f":exclamation: {ctx.author.mention} You are Already in a FootBall Match in this Server, use `>footballstop` to Stop your Current Match")
                else:
                    await ctx.send(f":exclamation: {player1.mention} is Already in a FootBall Match in this Server")
        else:
            if player2 == ctx.author and player1 == ctx.author:
                await ctx.send(f":exclamation: {ctx.author.mention} You Cannot Play With Yourself, There must be Two Players to Play Football")
            else:
                await ctx.send(f":exclamation: {ctx.author.mention} Single Player Cannot Play with Himself/Herself, There must be Two Players")
    else:
        await ctx.send(f":exclamation: {ctx.author.mention} You are Using The Command Wrong, Use `>help football` to get help related with the Command")

footballhelp = f"{prefix}football [First Player] <Second Player>"

@bot.command()
async def footballstop(ctx):
    global footballMatch, footballCode
    if ctx.author.id in footballMatch[ctx.guild.id].keys() and ctx.author.id in footballMatch[ctx.guild.id].values():
...
async def on_message(message):
    global footballMatch, footballCode
    if message.author.id in footballMatch[message.guild.id].keys() and message.author.id in footballMatch[message.guild.id].values():
...
async def help(ctx, anycommand: Optional[str]=None):
    ...
    if anycommand is None:
        ...
        myEmbed.add_field(name="Fun",value=" slap, kill, punch, tictactoe, tttstop, football, footballstop \n----------------------\n", inline=False)
        ...
        elif anycommand == "football": content=footballhelp
        elif anycommand == "footballstop": content=footballstophelp
